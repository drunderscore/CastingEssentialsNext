...
#define OFFSET_CHECK(className, memberName, expectedOffset)                                                            \
    static_assert(sizeof(CheckEqual<offsetof(className, memberName), expectedOffset>),                                 \
                  "Actual size differs from expected");
...
class OffsetChecking
{
    ...
    template<typename T>
    static void Check(int expectedOffset, T func)
    {
...
public:
    OffsetChecking()
    {
        ...
        // vgui::EditablePanel
        {
            ...
            Assert((offset = Hooking::VTableOffset(static_cast<void (EditablePanel::*)(const char*, float)>(
                        &EditablePanel::SetDialogVariable))) == 227);
            Assert((offset = Hooking::VTableOffset(static_cast<void (EditablePanel::*)(const char*, int)>(
                        &EditablePanel::SetDialogVariable))) == 228);
            Assert((offset = Hooking::VTableOffset(static_cast<void (EditablePanel::*)(const char*, const wchar_t*)>(
                        &EditablePanel::SetDialogVariable))) == 229);
            Assert((offset = Hooking::VTableOffset(static_cast<void (EditablePanel::*)(const char*, const char*)>(
                        &EditablePanel::SetDialogVariable))) == 230);
        }
