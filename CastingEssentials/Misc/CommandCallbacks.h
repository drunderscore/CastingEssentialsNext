...
{
public:
    CommandCallbacks(std::function<void(const CCommand& cmd)>&& dispatch) : m_Dispatch(std::move(dispatch)) {}
...
private:
    void CommandCallback(const CCommand& cmd) override final { m_Dispatch(cmd); }
