...
{
    if (!force_broadcast && value == s_Detected)
        return; // In general, don't re-send but we need it for initial load
    s_Detected = value;
    NotifyListeners();
}
...
{
    if (s_Detected)
        return; // Already detected, no need to enable
    if (!g_ProxyPusher.IsEmpty())
