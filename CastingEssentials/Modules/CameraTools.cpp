...
CameraTools::CameraTools()
    : ce_cameratools_show_mode("ce_cameratools_show_mode", "0", FCVAR_NONE,
                               "Displays the current spec_mode in the top right corner of the screen."),

      ce_cameratools_autodirector_mode("ce_cameratools_autodirector_mode", "0", FCVAR_NONE,
                                       "Forces the camera mode to this value.",
                                       [](IConVar* var, const char* pOldValue, float flOldValue) {
                                           GetModule()->ChangeForceMode(var, pOldValue, flOldValue);
                                       }),
      ce_cameratools_force_target("ce_cameratools_force_target", "-1", FCVAR_NONE,
                                  "Forces the camera target to this player index.",
                                  [](IConVar* var, const char* pOldValue, float flOldValue) {
                                      GetModule()->ChangeForceTarget(var, pOldValue, flOldValue);
                                  }),
      ce_cameratools_force_valid_target("ce_cameratools_force_valid_target", "0", FCVAR_NONE,
                                        "Forces the camera to only have valid targets.",
                                        [](IConVar* var, const char* pOldValue, float flOldValue) {
                                            GetModule()->ToggleForceValidTarget(var, pOldValue, flOldValue);
                                        }),
      ce_cameratools_spec_player_alive("ce_cameratools_spec_player_alive", "1", FCVAR_NONE,
                                       "Prevents spectating dead players."),
      ce_cameratools_fix_view_heights("ce_cameratools_fix_view_heights", "0", FCVAR_NONE,
                                      "Corrects HLTV view height to match that of spectated classes."),
      ce_cameratools_disable_view_punches(
          "ce_cameratools_disable_view_punches", "0", FCVAR_NONE,
      ...
      ce_tplock_enable("ce_tplock_enable", "0", FCVAR_NONE,
                       "Locks view angles in spec_mode 5 (thirdperson/chase) to always looking the same direction as "
                       "the spectated player."),
      ce_tplock_taunt_enable(
          "ce_tplock_taunt_enable", "0", FCVAR_NONE,
          "Force the camera into thirdperson tplock when taunting. Does not require ce_tplock_enable."),
      ...
      ce_tplock_default_pos("ce_tplock_default_pos", "18 -80 20", FCVAR_NONE,
                            ...
                            [](IConVar* var, const char* old, float) {
                                ParseTPLockValuesInto(static_cast<ConVar*>(var), old,
                                                      GetModule()->m_TPLockDefault.m_Pos);
                            }),
      ce_tplock_default_angle("ce_tplock_default_angle", "*0.5 ? =0", FCVAR_NONE,
                              "Camera angle offset (pitch/yaw/roll) from ce_tplock_bone when tplock is active. See "
                              "wiki for more information.",
                              [](IConVar* var, const char* old, float) {
                                  ParseTPLockValuesInto(static_cast<ConVar*>(var), old,
                                                        GetModule()->m_TPLockDefault.m_Angle);
                              }),
      ce_tplock_default_dps(
          "ce_tplock_default_dps", "-1 -1 -1", FCVAR_NONE,
      ...
      ce_tplock_taunt_pos("ce_tplock_taunt_pos", "0 -80 -15", FCVAR_NONE,
                          ...
                          [](IConVar* var, const char* old, float) {
                              ParseTPLockValuesInto(static_cast<ConVar*>(var), old, GetModule()->m_TPLockTaunt.m_Pos);
                          }),
      ce_tplock_taunt_angle(
          "ce_tplock_taunt_angle", "=15 180 =0", FCVAR_NONE,
          "Camera angle offset (pitch/yaw/roll) from ce_tplock_bone when taunting with taunt tplock enabled.",
          [](IConVar* var, const char* old, float) {
              ParseTPLockValuesInto(static_cast<ConVar*>(var), old, GetModule()->m_TPLockTaunt.m_Angle);
          }),
      ce_tplock_taunt_dps("ce_tplock_taunt_dps", "-1 -1 -1", FCVAR_NONE,
                          "Max degrees per second for angle (pitch/yaw/roll) when taunting with taunt tplock enabled. "
                          "Set < 0 to uncap.",
                          [](IConVar* var, const char* old, float) {
      ...
      ce_cameratools_spec_pos_delta(
          "ce_cameratools_spec_pos_delta", [](const CCommand& args) { GetModule()->SpecPositionDelta(args); },
          "Offsets the camera by the given values."),
      ce_cameratools_spec_class(
          "ce_cameratools_spec_class", [](const CCommand& args) { GetModule()->SpecClass(args); },
          "Spectates a specific class: ce_cameratools_spec_class <team> <class> [index]"),
      ce_cameratools_spec_steamid(
          "ce_cameratools_spec_steamid", [](const CCommand& args) { GetModule()->SpecSteamID(args); },
          "Spectates a player with the given steamid: ce_cameratools_spec_steamid <steamID>"),
      ce_cameratools_spec_index(
          "ce_cameratools_spec_index", [](const CCommand& args) { GetModule()->SpecIndex(args); },
          "Spectate a player based on their index in the tournament spectator hud."),

      ce_cameratools_show_users(
          "ce_cameratools_show_users", [](const CCommand& args) { GetModule()->ShowUsers(args); },
...
float CameraTools::CollisionTest3D(const Vector& startPos, const Vector& targetPos, float scale,
                                   const IHandleEntity* ignoreEnt)
{
    static constexpr Vector TEST_POINTS[27] = {
        Vector(0, 0, 0),     Vector(0, 0, 0.5),     Vector(0, 0, 1),

        Vector(0, 0.5, 0),   Vector(0, 0.5, 0.5),   Vector(0, 0.5, 1),

        Vector(0, 1, 0),     Vector(0, 1, 0.5),     Vector(0, 1, 1),

        Vector(0.5, 0, 0),   Vector(0.5, 0, 0.5),   Vector(0.5, 0, 1),

        Vector(0.5, 0.5, 0), Vector(0.5, 0.5, 0.5), Vector(0.5, 0.5, 1),

        Vector(0.5, 1, 0),   Vector(0.5, 1, 0.5),   Vector(0.5, 1, 1),

        Vector(1, 0, 0),     Vector(1, 0, 0.5),     Vector(1, 0, 1),

        Vector(1, 0.5, 0),   Vector(1, 0.5, 0.5),   Vector(1, 0.5, 1),

        Vector(1, 1, 0),     Vector(1, 1, 0.5),     Vector(1, 1, 1),
    };
...
void CameraTools::ShowUsers(const CCommand& command)
{
    ...
    for (size_t i = 0; i < red.size(); i++)
        ConColorMsg(Color(255, 128, 128, 255), "    alias player_red%i \"%s %s\"		// %s (%s)\n", i,
                    ce_cameratools_spec_steamid.GetName(),
                    RenderSteamID(red[i]->GetSteamID().ConvertToUint64()).c_str(), red[i]->GetName(),
                    TF_CLASS_NAMES[(int)red[i]->GetClass()]);

    for (size_t i = 0; i < blu.size(); i++)
        ConColorMsg(Color(128, 128, 255, 255), "    alias player_blu%i \"%s %s\"		// %s (%s)\n", i,
                    ce_cameratools_spec_steamid.GetName(),
                    RenderSteamID(blu[i]->GetSteamID().ConvertToUint64()).c_str(), blu[i]->GetName(),
                    TF_CLASS_NAMES[(int)blu[i]->GetClass()]);
}
...
void CameraTools::SpecClass(TFTeam team, TFClassType playerClass, int classIndex)
{
    ...
    if (classIndex < 0)
    {
        ...
        if (localMode == OBS_MODE_FIXED || localMode == OBS_MODE_IN_EYE || localMode == OBS_MODE_CHASE)
        {
...
void CameraTools::OnTick(bool inGame)
{
    ...
    if (inGame)
    {
        if (ce_cameratools_show_mode.GetBool())
        {
            ...
            if (mode >= 0)
            {
                Interfaces::GetEngineClient()->Con_NPrintf(
                    GetConLine(), "Current spec_mode: %i %s", mode,
                    mode >= 0 && mode < NUM_OBSERVER_MODES ? s_ObserverModes[mode] : "INVALID");
                Interfaces::GetEngineClient()->Con_NPrintf(
                    GetConLine(), "Current target: %i%s", target,
                    target == 0 ? " (Unspecified)" : (strprintf(" (%s)", playerName.c_str())).c_str());
            }
        }
...
bool CameraTools::FixViewHeights()
{
    ...
    auto& viewOffsetVar =
        *reinterpret_cast<CInterpolatedVar<Vector>*>(reinterpret_cast<uintptr_t>(basePlayer) + 0x1698);
...
void CameraTools::ChangeForceMode(IConVar* var, const char* pOldValue, float flOldValue)
{
    ...
    if (forceMode == OBS_MODE_FIXED || forceMode == OBS_MODE_IN_EYE || forceMode == OBS_MODE_CHASE ||
        forceMode == OBS_MODE_ROAMING)
    {
        try
        {
...
bool CameraTools::ParseSpecPosCommand(const CCommand& command, Vector& pos, QAngle& ang, ObserverMode& mode,
                                      const Vector& defaultPos, const QAngle& defaultAng,
                                      ObserverMode defaultMode) const
{
...
PrintUsage:
    PluginMsg("Usage: %s x [y] [z] [pitch] [yaw] [roll] [mode]\n"
