...
AutoCameras::AutoCameras()
    : ce_cameratrigger_begin(
          "ce_autocamera_trigger_begin", [](const CCommand& args) { GetModule()->BeginCameraTrigger(); }, nullptr,
          FCVAR_UNREGISTERED),
      ce_cameratrigger_end(
          "ce_autocamera_trigger_end", [](const CCommand& args) { GetModule()->EndCameraTrigger(); }, nullptr,
          FCVAR_UNREGISTERED),

      ce_autocamera_begin_storyboard(
          "ce_autocamera_begin_storyboard", [](const CCommand& args) { GetModule()->BeginStoryboard(args); }, nullptr,
          FCVAR_UNREGISTERED),

      ce_autocamera_create("ce_autocamera_create", [](const CCommand& args) { GetModule()->MarkCamera(args); }),
      ce_autocamera_goto(
          "ce_autocamera_goto", [](const CCommand& args) { GetModule()->GotoCamera(args); }, nullptr, 0,
          &AutoCameras::GotoCameraCompletion),
      ce_autocamera_goto_mode("ce_autocamera_goto_mode", "3", FCVAR_NONE,
                              "spec_mode to use for ce_autocamera_goto and ce_autocamera_cycle.", true, 0, true,
                              NUM_OBSERVER_MODES - 1),

      ce_autocamera_cycle_debug("ce_autocamera_cycle_debug", "0"),
      ce_autocamera_cycle(
          "ce_autocamera_cycle", [](const CCommand& args) { GetModule()->CycleCamera(args); },
          "\nUsage: ce_autocamera_cycle <next/prev> [group]. Cycles forwards or backwards logically through the "
          "available autocameras. The behavior is as follows:\n"
          "\tIf already spectating from the position of an autocamera, progress forwards/backwards through the "
          "autocameras based on file order.\n"
          "\tIf our current position doesn't match any autocamera definintions, try to find the nearest/furthest "
          "autocamera that has line of sight (LOS) to our current position.\n"
          "\tIf no autocameras have LOS to our current position, find the nearest/furthest autocamera with our current "
          "position in its view frustum.\n"
          "\tIf our position is not in any autocamera's view frustum, find the nearest/furthest autocamera to our "
          "current position."),

      ce_autocamera_spec_player(
          "ce_autocamera_spec_player", [](const CCommand& args) { GetModule()->SpecPlayer(args); },
          "Switches to an autocamera with the best view of the current player. Usage: ce_autocamera_spec_player "
          "[group]."),
      ce_autocamera_spec_player_fallback(
          "ce_autocamera_spec_player_fallback", "firstperson", FCVAR_NONE,
          "Behavior to fall back to when using ce_autocamera_spec_player and no autocameras make it through the "
          "filters. Valid values: firstperson/thirdperson/none."),
      ce_autocamera_spec_player_fov(
          "ce_autocamera_spec_player_fov", "75", FCVAR_NONE,
          "FOV to use when checking if a player is in view of an autocamera. -1 to disable check."),
      ce_autocamera_spec_player_dist("ce_autocamera_spec_player_dist", "2000", FCVAR_NONE,
                                     "Maximum distance a player can be from an autocamera."),
      ...
      ce_autocamera_reload_config(
          "ce_autocamera_reload_config", []() { GetModule()->LoadConfig(); },
          "Reloads the autocamera definitions file (located in "
          "/tf/addons/castingessentials/autocameras/<mapname>.vdf)"),

      ce_autocamera_show_triggers("ce_autocamera_show_triggers", "0", FCVAR_UNREGISTERED,
                                  "Shows all triggers on the map."),
...
void AutoCameras::LevelInit() { LoadConfig(); }
...
bool AutoCameras::LoadCameraGroup(KeyValues* group)
{
    ...
    for (KeyValues* camNameValue = group->GetFirstValue(); camNameValue; camNameValue = camNameValue->GetNextValue())
    {
        ...
        if (!found)
        {
            PluginWarning("CameraGroup \"%s\" in \"%s\" referenced unknown camera \"%s\"", newGroup->m_Name.c_str(),
                          m_ConfigFilename.c_str(), camName);
            continue;
        }
...
bool AutoCameras::LoadShot(std::unique_ptr<StoryboardElement>& shotOut, KeyValues* const shotKV,
                           const char* const storyboardName, const char* const filename)
{
...
bool AutoCameras::LoadAction(std::unique_ptr<StoryboardElement>& actionOut, KeyValues* const actionKV,
                             const char* const storyboardName, const char* const filename)
{
...
void AutoCameras::EndCameraTrigger()
{
    if (!m_CreatingCameraTrigger)
    {
        PluginWarning("Must use %s before using %s!\n", ce_cameratrigger_begin.GetName(),
                      ce_cameratrigger_end.GetName());
        return;
    }
    ...
    ConColorMsg(Color(128, 255, 128, 255), "Trigger { mins \"%1.1f %1.1f %1.1f\" maxs \"%1.1f %1.1f %1.1f\" }\n",
                m_CameraTriggerStart.x, m_CameraTriggerStart.y, m_CameraTriggerStart.z, cameraTriggerEnd.x,
                cameraTriggerEnd.y, cameraTriggerEnd.z);
...
void AutoCameras::SpecPlayer(const CCommand& args)
{
    ...
    if (bestCameraScore <= 0)
    {
        ...
        if (auto fallback = ce_autocamera_spec_player_fallback.GetString(); fallback[0])
        {
            ...
            {
                PluginWarning("%s: unknown fallback mode \"%s\"\n", ce_autocamera_spec_player_fallback.GetName(),
                              fallback);
                return;
            }
...
float AutoCameras::ScoreSpecPlayerCamera(const Camera& camera, const Vector& position,
                                         const IHandleEntity* targetEnt) const
{
    float score = 1;
    ...
    if (const float dist = ce_autocamera_spec_player_dist.GetFloat(); score > 0 && dist > 0)
    {
        ...
        score *=
            RemapValClamped(positionDist, minDistEnd, minDistBegin, 0, 1) * RemapVal(positionDist, 256, dist, 1, 0);
    }
...
int AutoCameras::GotoCameraCompletion(const char* const partial,
                                      char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH])
{
    std::cmatch match;
    const std::string regexString = strprintf("\\s*%s\\s+\"?(.*?)\"?\\s*$", GetModule()->ce_autocamera_goto.GetName());

    // Interesting... this only works if you pass a const char* to std::regex's constructor, and not if you pass an
    // std::string.
...
void AutoCameras::DrawCameras()
{
    ...
    for (const auto& camera : m_Cameras)
    {
        ...
        {
            NDebugOverlay::BoxAngles(camera->m_Pos, Vector(-16, -16, -16), Vector(16, 16, 16), camera->m_DefaultAngle,
                                     128, 128, 255, 64, 0);
...
void AutoCameras::SetupMirroredCameras()
{
    ...
    do
    {
        ...
        for (const auto& camera : m_Cameras)
        {
            ...
            if (!camToMirror)
            {
                if (FindContainedString(m_MalformedCameras, camera->m_MirroredCameraName.c_str()))
                    PluginWarning("Unable to mirror camera \"%s\" because its base \"%s\" is malformed in \"%s\".\n",
                                  camera->m_Name.c_str(), camera->m_MirroredCameraName.c_str(),
                                  m_ConfigFilename.c_str());
                else
                    PluginWarning("Unable to mirror camera \"%s\" because the base camera definition \"%s\" cannot be "
                                  "found in \"%s\".\n",
                                  camera->m_Name.c_str(), camera->m_MirroredCameraName.c_str(),
                                  m_ConfigFilename.c_str());

                // This camera is no good, so make sure it never gets treated as a mirror target. Also mark is as
                // malformed.
            ...
            cameraEdit->m_Pos =
                m_MapOrigin + Vector((cameraEdit->m_MirrorX ? -1.0 : 1.0) * relativeBasePos.x,
                                     (cameraEdit->m_MirrorY ? -1.0 : 1.0) * relativeBasePos.y, relativeBasePos.z);
            ...
            cameraEdit->m_DefaultAngle = QAngle(AngleNormalize(camToMirror->m_DefaultAngle.x), AngleNormalize(angle),
