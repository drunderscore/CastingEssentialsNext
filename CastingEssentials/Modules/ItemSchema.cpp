...
ItemSchema::ItemSchema()
    : ce_itemschema_print(
          "ce_itemschema_print", []() { GetModule()->PrintAliases(); },
          "Prints the current state of the item schema module."),
      ce_itemschema_reload(
          "ce_itemschema_reload", []() { GetModule()->LoadItemSchema(); }, "Reloads the item schema module.")
{
}
...
void ItemSchema::PrintAliases()
{
    ...
    for (const auto& unique : m_UniqueItems)
    {
        ...
        for (const auto& mapped : unique.m_Entries)
        {
            ...
            ConColorMsg(randColor, "\tMapped item: %s (id %u) %s\n", mapped.m_Name.c_str(), mapped.m_ID,
                        mapped.GetPlayerModel().c_str());
        }
...
void ItemSchema::LevelInit() { LoadItemSchema(); }
...
void ItemSchema::LoadItemSchema()
{
    ...
    PluginMsg("Generated item schema mappings: %zu items down to %zu.\n", itemsCount,
              std::distance(m_UniqueItems.begin(), m_UniqueItems.end()));
...
void ItemSchema::LoadItems(KeyValues* inItems, std::forward_list<ItemEntry>& outItems)
{
    ...
    do
    {
        ...
        if (newItem.GetEquipSlot() != ItemEquipSlot::Primary && newItem.GetEquipSlot() != ItemEquipSlot::Secondary &&
                newItem.GetEquipSlot() != ItemEquipSlot::Melee && newItem.GetEquipSlot() != ItemEquipSlot::PDA &&
                newItem.GetEquipSlot() != ItemEquipSlot::PDA2 && newItem.GetEquipSlot() != ItemEquipSlot::Building ||
        ...
        else
        {
            ...
            for (const auto& item : items)
            {
                ...
                if (stristr(playerModel.c_str(), value))
                {
                    ...
                    {
                        PluginWarning("[Item Schema Overrides] Target weapon model \"%s\" matches 2 or more items. Try "
                                      "making the override more specific.\n",
                                      value);
                        continue;
                    }
            ...
            if (!foundString)
                PluginWarning("[Item Schema Overrides] Unable to find a target weapon using a model containing the "
                              "string \"%s\".\n",
                              value);
        }
