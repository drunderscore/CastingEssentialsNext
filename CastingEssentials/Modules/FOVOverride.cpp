...
FOVOverride::FOVOverride()
    : ce_fovoverride_firstperson("ce_fovoverride_firstperson", "90"),
      ...
      ce_fovoverride_roaming("ce_fovoverride_roaming", "90"), ce_fovoverride_fixed("ce_fovoverride_fixed", "90"),
      ce_fovoverride_test("ce_fovoverride_test", "90", FCVAR_NONE,
                          "FOV override to apply in all cases. Only enabled if ce_fovoverride_all_enabled is nonzero. "
                          "Designed to help with the placement of autocameras."),
...
float FOVOverride::GetBaseFOV(ObserverMode mode) const
{
    ...
    switch (mode)
    {
        case OBS_MODE_IN_EYE:
            fov = ce_fovoverride_firstperson.GetFloat();
            break;
        case OBS_MODE_CHASE:
            fov = ce_fovoverride_thirdperson.GetFloat();
            break;
        case OBS_MODE_FIXED:
            fov = ce_fovoverride_fixed.GetFloat();
            break;
        case OBS_MODE_ROAMING:
            fov = ce_fovoverride_roaming.GetFloat();
            break;
    }
...
bool FOVOverride::InToolModeOverride() const { return true; }
