...
ConsoleTools::ConsoleTools()
    : ce_consoletools_filter_enabled(
      ...
      ce_consoletools_filter_remove(
          "ce_consoletools_filter_remove", [](const CCommand& cmd) { GetModule()->RemoveFilter(cmd); },
          "Removes an existing console filter."),
      ce_consoletools_filter_list(
          "ce_consoletools_filter_list", [](const CCommand& cmd) { GetModule()->ListFilters(cmd); },
          "Lists all console filters."),

      ce_consoletools_flags_add("ce_consoletools_flags_add", m_AddFlagsCallbacks, "Adds a flag to a cvar.", FCVAR_NONE,
                                m_AddFlagsCallbacks),
      ce_consoletools_flags_remove("ce_consoletools_flags_remove", m_RemoveFlagsCallbacks,
                                   "Removes a flag from a cvar.", FCVAR_NONE, m_RemoveFlagsCallbacks),

      ce_consoletools_limits_set("ce_consoletools_limits_set", m_SetLimitsCallbacks,
                                 "Sets or removes limits for a ConVar.", FCVAR_NONE, m_SetLimitsCallbacks),

      ce_consoletools_alias_remove("ce_consoletools_alias_remove", m_RemoveAliasCallbacks,
                                   "Removes an existing alias created with the \"alias\" command.", FCVAR_NONE,
                                   m_RemoveAliasCallbacks),
      ...
      m_ConsoleColorPrintfHook(
          std::bind(&ConsoleTools::ConsoleColorPrintfHook, this, std::placeholders::_1, std::placeholders::_2)),
      ...
      m_SetLimitsCallbacks(&SetLimits, &SetLimitsAutocomplete), m_AddFlagsCallbacks(&AddFlags, &FlagModifyAutocomplete),
...
Usage:
    Warning(
        "Usage: %s <cvar> <minimum/'x' for none/'?' for keep existing> <maximum/'x' for none/'?' for keep existing>\n",
        command[0]);
}
...
int ConsoleTools::ParseFlags(const CCommand& command)
{
    ...
    for (int i = 2; i < command.ArgC(); i++)
    {
        ...
        else if (!stricmp(flag, "devonly") || !stricmp(flag, "developmentonly") ||
                 !stricmp(flag, "FCVAR_DEVELOPMENTONLY"))
