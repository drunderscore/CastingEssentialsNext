...
const HUDHacking::ChargeBarInfo HUDHacking::s_ChargeBarInfo[(int)ChargeBarType::COUNT] = {
    ...
    ChargeBarInfo("medigun_kritz", 1, "the Kritzkrieg", "#TF_Unique_Achievement_Medigun1", 35,
                  MeterType::MedigunCharge),
    ChargeBarInfo("medigun_quickfix", 1, "the Quick-Fix", "#TF_Unique_MediGun_QuickFix", 411, MeterType::MedigunCharge),
    ChargeBarInfo("medigun_vaccinator", 1, "the Vaccinator", "#TF_Unique_MediGun_Resist", 998,
                  MeterType::MedigunCharge),
    ...
    ChargeBarInfo("cloak_and_dagger", 1, "the Cloak and Dagger", "#TF_Unique_Achievement_CloakWatch", 60,
                  MeterType::Cloak),
...
ConVar HUDHacking::ce_hud_debug_unassociated_playerpanels(
    "ce_hud_debug_unassociated_playerpanels", "0", FCVAR_NONE,
    "Print debug messages to the console when a player cannot be found for a given playerpanel.");

HUDHacking::HUDHacking()
    : ce_hud_forward_playerpanel_border(
          "ce_hud_forward_playerpanel_border", "0", FCVAR_NONE,
          "Sets the border of [playerpanel]->PanelColorBG to the same value as [playerpanel]."),
      ce_hud_player_health_progressbars("ce_hud_player_health_progressbars", "0", FCVAR_NONE,
                                        "Enables [playerpanel]->PlayerHealth[Overheal](Red/Blue) ProgressBars."),
      ce_hud_player_status_effects("ce_hud_player_status_effects", "0", FCVAR_NONE,
                                   "Update status effect ImagePanel: [playerpanel]->StatusEffectIcon(Red/Blue)"),
      ce_hud_player_status_effects_debug("ce_hud_player_status_effects_debug", "0", FCVAR_NONE,
                                         "Shows a status effect icon for all players.", true, 0, true,
      ...
      ce_hud_progressbar_directions(
          "ce_hud_progressbar_directions", "0", FCVAR_NONE,
          "Enables setting 'direction <north/east/south/west>' for vgui ProgressBar elements.",
          [](IConVar* var, const char*, float) {
              GetModule()->m_ApplySettingsHook.SetEnabled(static_cast<ConVar*>(var)->GetBool());
          }),

      ce_hud_find_parent_elements("ce_hud_find_parent_elements", "0", FCVAR_NONE,
                                  "Enables moving a panel's search-by-name scope upwards by prefixing the name with "
                                  "'../' (like referencing a parent's sibling in a hud animation)",
                                  [](IConVar* var, const char*, float) {
                                      GetModule()->m_FindChildByNameHook.SetEnabled(
                                          static_cast<ConVar*>(var)->GetBool());
                                  }),

      ce_hud_class_change_animations(
          "ce_hud_class_change_animations", "0", FCVAR_NONE,
      ...
      m_FindChildByNameHook(std::bind(&HUDHacking::FindChildByNameOverride, this, std::placeholders::_1,
                                 ...
                                 const IClientNetworkable& weapon) const
{
    if (ce_hud_chargebars_debug.GetBool())
    {
...

        {
            ...
            for (const auto& prop :
                 EntityOffsetIterable<float>(&weapon, std::begin(excludeTables), std::end(excludeTables)))
        ...
        for (int a = 0; a < 11; a++)
            engine->Con_NPrintf(i++, "item charge %i: %1.2f", a,
                                s_ItemChargeMeters[a].TryGetValue(&playerNetworkable, NAN));
    }

    switch (info.m_Meter)
    {
        case MeterType::Rage:
            return s_RageMeter.GetValue(&playerNetworkable);
        case MeterType::EnergyDrink:
            return s_EnergyDrinkMeter.GetValue(&playerNetworkable);
        case MeterType::Hype:
            return s_HypeMeter.GetValue(&playerNetworkable);
        case MeterType::Charge:
            return s_ChargeMeter.GetValue(&playerNetworkable);
        case MeterType::MedigunCharge:
            return s_MedigunChargeMeter.GetValue(&weapon) * 100;
        case MeterType::Cloak:
            return s_CloakMeter.GetValue(&playerNetworkable);

        case MeterType::ItemCharge0:
            return s_ItemChargeMeters[0].GetValue(&playerNetworkable);
        case MeterType::ItemCharge1:
            return s_ItemChargeMeters[1].GetValue(&playerNetworkable);
        case MeterType::CleanersCarbine:
            return player.GetState<PlayerState>().GetCleanersCarbineCharge();
...
const char* HUDHacking::GetStatusEffectFormatString(StatusEffect effect)
{
    switch (effect)
    {
        case StatusEffect::None:
            return nullptr;
        case StatusEffect::Ubered:
            return "%subered_%s";
        case StatusEffect::Kritzed:
            return "%skritzkrieged_%s";
        case StatusEffect::Quickfixed:
            return "%squickfixed_%s";
        case StatusEffect::VaccinatorBullet:
            return "%svaccinated_%s_bullet";
        case StatusEffect::VaccinatorBlast:
            return "%svaccinated_%s_explosive";
        case StatusEffect::VaccinatorFire:
            return "%svaccinated_%s_fire";
        case StatusEffect::BuffBanner:
            return "%sbuff_banner_%s";
        case StatusEffect::Concheror:
            return "%sconcheror_%s";
        case StatusEffect::Battalions:
            return "%sbattalions_backup_%s";
        case StatusEffect::Bleeding:
            return "%sbleeding_%s";
        case StatusEffect::MarkedForDeath:
            return "%smarked_for_death_%s";
        case StatusEffect::Burning:
            return "%sburning_%s";
        case StatusEffect::Bonked:
            return "%sbonked_%s";
        case StatusEffect::Jarated:
            return "%sjarated_%s";
        case StatusEffect::Milked:
            return "%smilked_%s";
    }
...
HUDHacking::StatusEffect HUDHacking::GetStatusEffect(const Player& player)
{
    if (player.IsAlive())
    {
        ...
        else if (player.CheckCondition(TFCond::TFCond_MarkedForDeath) ||
                 player.CheckCondition(TFCond::TFCond_MarkedForDeathSilent))
...
void HUDHacking::UpdatePlayerPanels()
{
    ...
    for (int playerPanelIndex = 0; playerPanelIndex < specguiChildCount; playerPanelIndex++)
    {
        ...
        if (!g_pVGuiPanel->IsVisible(playerVPanel) ||
            strncmp(playerPanelName, "playerpanel", 11)) // Names are like "playerpanel13"
            continue;

        vgui::EditablePanel* playerPanel =
            assert_cast<vgui::EditablePanel*>(g_pVGuiPanel->GetPanel(playerVPanel, "ClientDLL"));
        if (!playerPanel)
            continue;
...
void HUDHacking::UpdateStatusEffect(vgui::VPANEL playerVPanel, vgui::EditablePanel* playerPanel, const Player& player)
{
    ...
    icon = dynamic_cast<vgui::ImagePanel*>(
        FindChildByName(playerVPanel, isRedTeam ? "StatusEffectIconRed" : "StatusEffectIconBlue"));
    if (!icon)
        return;
...
void HUDHacking::UpdateChargeBar(bool enabled, vgui::VPANEL playerVPanel, vgui::EditablePanel* playerPanel,
                                 Player& player) const
{
    if (!enabled)
    {
    ...
    if (shouldShowInfo)
    {
        ...
        sprintf_s(iconPath, "../castingessentials/chargebars/%s_%s", s_ChargeBarInfo[(int)type].m_Name,
                  isRedTeam ? "red" : "blue");

        if (C_BaseCombatWeapon * medigun;
            type == ChargeBarType::MedigunVaccinator && (medigun = player.GetMedigun()) != nullptr)
        {
            strcat_s(iconPath, "_");
    ...
    if (auto chargebar = dynamic_cast<vgui::ContinuousProgressBar*>(
            FindChildByName(playerVPanel, isRedTeam ? "WeaponChargeRed" : "WeaponChargeBlue")))
        chargebar->SetVisible(shouldShowInfo);

    if (auto icon = dynamic_cast<vgui::ImagePanel*>(
            FindChildByName(playerVPanel, isRedTeam ? "WeaponChargeIconRed" : "WeaponChargeIconBlue")))
        icon->SetImage(iconPath);
}
#pragma warning(pop)

void HUDHacking::UpdateClassChangeAnimations(vgui::VPANEL playerVPanel, vgui::EditablePanel* playerPanel,
                                             Player& player)
{
...
void HUDHacking::CheckItemForCharge(Player& player, const IClientNetworkable& playerNetworkable,
                                    const IClientNetworkable& item, int& bestPriority, ChargeBarType& type,
                                    float& charge) const
{
...
bool HUDHacking::GetChargeBarData(Player& player, ChargeBarType& type, float& charge) const
{
    if (false)
    {
        ...
        for (const auto& prop : EntityOffsetIterable<int>(player.GetEntity()->GetClientClass()->m_pRecvTable,
                                                          std::begin(excludeTables), std::end(excludeTables)))
    ...
    {
        C_BaseEntity* item = HookManager::GetRawFunc<HookFunc::C_TFPlayer_GetEntityForLoadoutSlot>()(
            (C_TFPlayer*)player.GetEntity(), iWearable, true);
        if (!item)
            continue;
...
float HUDHacking::PlayerState::GetCleanersCarbineCharge() const
{
    if (m_CleanersCarbineEmptyTime > 0)
        return std::clamp<float>((m_CleanersCarbineEmptyTime - Interfaces::GetEngineTool()->ClientTime()) /
                                     CLEANERS_CARBINE_DURATION,
                                 0, 1) *
               100;
    else
...
HUDHacking::WeaponChargeBarCvarData::WeaponChargeBarCvarData(const ChargeBarInfo& info)
    : m_PriorityCvarName(FormatAndAlloc("ce_hud_chargebars_priority_%s", info.m_Name)),
      m_PriorityCvarHelpText(FormatAndAlloc("Priority of the charge bar for %s when competing with other enabled "
                                            "chargebars. 0 to disable showing this charge bar.",
                                            info.m_HelpText)),
      ...
      m_TextCvarHelpText(
          FormatAndAlloc("Text to display on the hud when the chargebar for %s is showing.", info.m_HelpText)),
