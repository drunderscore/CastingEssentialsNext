...
private:
    ...
    bool LoadShot(std::unique_ptr<StoryboardElement>& shot, KeyValues* shotKV, const char* storyboardName,
                  const char* filename);
    bool LoadAction(std::unique_ptr<StoryboardElement>& action, KeyValues* actionKV, const char* storyboardName,
                    const char* filename);
    ...
    float ScoreSpecPlayerCamera(const Camera& camera, const Vector& position,
                                const IHandleEntity* targetEnt = nullptr) const;
    ...
    static int GotoCameraCompletion(const char* partial,
