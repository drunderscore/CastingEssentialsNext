...
Graphics::Graphics()
    : ce_graphics_disable_prop_fades(
          "ce_graphics_disable_prop_fades", "0", FCVAR_NONE, "Enable/disable prop fading.",
      ...
      ce_graphics_improved_glows(
          "ce_graphics_improved_glows", "1", FCVAR_NONE, "Should we used the new and improved glow code?",
          [](IConVar* var, const char*, float) { GetModule()->ToggleImprovedGlows(static_cast<ConVar*>(var)); }),
      ce_graphics_fix_invisible_players(
          "ce_graphics_fix_invisible_players", "1", FCVAR_NONE,
          "Fix a case where players are invisible if you're firstperson speccing them when the round starts."),
      ce_graphics_fix_viewmodel_particles(
          "ce_graphics_fix_viewmodel_particles", "1", FCVAR_NONE,
          "Fix a case where particles on view models(e.g. medic beams and unusual effects) disappear right after "
          "loading.",
      ...
      ce_outlines_debug_stencil_out("ce_outlines_debug_stencil_out", "1", FCVAR_NONE,
                                    "Should we stencil out the players during the final blend to screen?"),
      ce_outlines_players_override_red(
          "ce_outlines_players_override_red", "", FCVAR_NONE,
      ...
      ce_outlines_spy_visibility(
          "ce_outlines_spy_visibility", "1", FCVAR_NONE,
          "If set to 1, always show outlines around cloaked spies (as opposed to only when they are behind walls)."),
      ce_outlines_cull_frustum("ce_outlines_cull_frustum", "1", FCVAR_HIDDEN,
                               "Enable frustum culling for outlines/infills."),
      ce_outlines_pvs_optimizations("ce_outlines_pvs_optimizations", "1", FCVAR_HIDDEN,
                                    "Simplifies glow when occluded/unoccluded outlines for entities based on PVS."),
      ...
      ce_infills_hurt_red("ce_infills_hurt_red", "189 59 59 96", FCVAR_NONE,
                          "Infill for red players that are not overhealed."),
      ce_infills_hurt_blue("ce_infills_hurt_blue", "91 122 140 96", FCVAR_NONE,
                           "Infill for blue players that are not overhealed."),
      ce_infills_buffed_red("ce_infills_buffed_red", "240 129 73 0", FCVAR_NONE,
                            "Infill for red players that are overhealed."),
      ce_infills_buffed_blue("ce_infills_buffed_blue", "239 152 73 0", FCVAR_NONE,
                             "Infill for blue players that are overhealed."),
      ce_infills_debug("ce_infills_debug", "0", FCVAR_NONE),
      ce_infills_test(
          "ce_infills_test", []() { GetModule()->ResetPlayerHurtTimes(); },
          "Replay the hurt flicker/fades for all players for testing."),

      ce_infills_flicker_hertz("ce_infills_flicker_hertz", "10", FCVAR_NONE, "Infill on-hurt flicker frequency.", true,
                               0, false, 1),
      ce_infills_flicker_intensity("ce_infills_flicker_intensity", "0.55", FCVAR_NONE,
                                   "Infill on-hurt flicker intensity", true, 0, true, 1),
      ...
      ce_infills_flicker_after_hurt_bias("ce_infills_flicker_after_hurt_bias", "0.15", FCVAR_NONE,
                                         "Bias amount for flicker fade outs.", true, 0, true, 1),
      ce_infills_fade_after_hurt_time("ce_infills_fade_after_hurt_time", "2.0", FCVAR_NONE,
                                      "How long to fade out infills after taking damage. -1 to disable."),
      ce_infills_fade_after_hurt_bias("ce_infills_fade_after_hurt_bias", "0.15", FCVAR_NONE,
                                      "Bias amount for infill fade outs.", true, 0, true, 1),

      m_ShaderParamsCallbacks(&DumpShaderParams, &DumpShaderParamsAutocomplete),
      ce_graphics_dump_shader_params("ce_graphics_dump_shader_params", m_ShaderParamsCallbacks,
                                     "Prints out all parameters for a given shader.", FCVAR_NONE,
                                     m_ShaderParamsCallbacks),
      ce_graphics_dump_rts("ce_graphics_dump_rts", DumpRTs, "Dump all rendertargets to console."),

      m_PostEffectsHook(std::bind(&Graphics::PostEffectsOverride, this, std::placeholders::_1, std::placeholders::_2,
                                  std::placeholders::_3, std::placeholders::_4, std::placeholders::_5)),

      m_ComputePropOpacityHook(
          std::bind(&Graphics::ComputePropOpacityOverride, this, std::placeholders::_1, std::placeholders::_2)),
      m_ComputeEntityFadeHook(std::bind(&Graphics::ComputeEntityFadeOveride, this, std::placeholders::_1,
                                        std::placeholders::_2, std::placeholders::_3, std::placeholders::_4)),

      m_ApplyEntityGlowEffectsHook(std::bind(&Graphics::ApplyEntityGlowEffectsOverride, this, std::placeholders::_1,
                                             std::placeholders::_2, std::placeholders::_3, std::placeholders::_4,
                                             std::placeholders::_5, std::placeholders::_6, std::placeholders::_7,
                                             std::placeholders::_8, std::placeholders::_9)),

      m_ForcedMaterialOverrideHook(
          std::bind(&Graphics::ForcedMaterialOverrideOverride, this, std::placeholders::_1, std::placeholders::_2)),

      m_PostDataUpdateHook(
          std::bind(&Graphics::PostDataUpdateOverride, this, std::placeholders::_1, std::placeholders::_2)),
...
{
    return !stricmp(paramName, "$alpha") || !stricmp(paramName, "$basetexturetransform") ||
           !stricmp(paramName, "$basetexture") || !stricmp(paramName, "$color") || !stricmp(paramName, "$color2") ||
           !stricmp(paramName, "$flags") || !stricmp(paramName, "$flags_defined") || !stricmp(paramName, "$flags2") ||
           !stricmp(paramName, "$flags_defined2") || !stricmp(paramName, "$flashlighttexture") ||
           !stricmp(paramName, "$flashlighttextureframe") || !stricmp(paramName, "$srgbtint") ||
...
void Graphics::DumpShaderParams(const CCommand& cmd)
{
    ...
    if (cmd.ArgC() < 2)
    {
        ...
        std::sort(shaderList.begin(), shaderList.end(),
                  [](const IShader* p1, const IShader* p2) { return stricmp(p1->GetName(), p2->GetName()) < 0; });
    ...
    for (const auto& current : shaderList)
    {
        ...
        paramOrder.reserve(paramCount);
        {
            ...
            std::sort(paramOrder.begin(), paramOrder.end(), [current](int p1, int p2) {
                return stricmp(current->GetParamName(p1), current->GetParamName(p2)) < 0;
            });
        }
        ...
        for (int p2 = 0; p2 < paramCount; p2++)
        {
            ...
            ConColorMsg(clr, "\t%s (%s): %s - default \"%s\"\n", name, current->GetParamHelp(p), type,
                        current->GetParamDefault(p));
        }
...
void Graphics::ToggleFXAA(const ConVar* var) { m_PostEffectsHook.SetEnabled(var->GetBool()); }
...
void Graphics::DrawFXAA(int x, int y, int w, int h)
{
    ...
    {
        CRefPtrFix<IMaterial> lumaToAlphaMaterial(
            materials->FindMaterial("castingessentials/fxaa/luma_to_alpha", TEXTURE_GROUP_CLIENT_EFFECTS));
        ...
        {
            pRenderContext->DrawScreenSpaceRectangle(lumaToAlphaMaterial, x, y, w, h, x, y, x + w - 1, y + h - 1,
    ...
    // FXAA to backbuffer
    {
        CRefPtrFix<IMaterial> fxaaMaterial(
            materials->FindMaterial("castingessentials/fxaa/fxaa", TEXTURE_GROUP_CLIENT_EFFECTS));
        ...
        pRenderContext->DrawScreenSpaceRectangle(fxaaMaterial, x, y, w, h, x, y, x + w - 1, y + h - 1,
...
void Graphics::ApplyEntityGlowEffectsOverride(CGlowObjectManager* pThis, const CViewSetup* pSetup, int nSplitScreenSlot,
                                              CMatRenderContextPtr& pRenderContext, float flBloomScale, int x, int y,
                                              int w, int h)
{
...
bool Graphics::ShouldDrawLocalPlayerOverride(C_BasePlayer* pThis)
{
    ...
    if (!ret && CameraState::GetLocalObserverMode() == OBS_MODE_IN_EYE)
    {
        ...
        if (specTarget && m_LocalOwner == specTarget->GetEntity())
            return true;
    }
    return ret;
}
...
void Graphics::GetAABBCorner(const Vector& mins, const Vector& maxs, uint_fast8_t cornerIndex, Vector& corner)
{
    ...
    corner.Init(cornerIndex & (1 << 2) ? maxs.x : mins.x, cornerIndex & (1 << 1) ? maxs.y : mins.y,
                cornerIndex & (1 << 0) ? maxs.z : mins.z);
}

void Graphics::GetRotatedBBCorners(const Vector& origin, const QAngle& angles, const Vector& mins, const Vector& maxs,
                                   Vector corners[8])
{
...
{
    float w = worldToScreen[3][0] * world[0] + worldToScreen[3][1] * world[1] + worldToScreen[3][2] * world[2] +
              worldToScreen[3][3];
    if (w < 0.001f)
        return false;
    ...
    screen.Init((worldToScreen[0][0] * world[0] + worldToScreen[0][1] * world[1] + worldToScreen[0][2] * world[2] +
                 worldToScreen[0][3]) *
                    invW,
                (worldToScreen[1][0] * world[0] + worldToScreen[1][1] * world[1] + worldToScreen[1][2] * world[2] +
                 worldToScreen[1][3]) *
                    invW);
...
float Graphics::ApplyInfillTimeEffects(float lastHurtTime)
{
    ...
    const auto fadeProgress = ce_infills_fade_after_hurt_time.GetFloat() > 0
                                  ? RemapValClamped(lastHurtTime, 0, ce_infills_fade_after_hurt_time.GetFloat(), 0, 1)
                                  : 0;

    const float fade = 1 - Bias(fadeProgress, ce_infills_fade_after_hurt_bias.GetFloat());

    const auto flickerProgress =
        ce_infills_flicker_after_hurt_time.GetFloat() > 0
            ? RemapValClamped(lastHurtTime, 0, ce_infills_flicker_after_hurt_time.GetFloat(), 0, 1)
            : 0;
...
void Graphics::BuildExtraGlowData(CGlowObjectManager* glowMgr, bool& anyAlways, bool& anyOccluded, bool& anyUnoccluded)
{
    ...
    GeneratePerspectiveFrustum(m_View->origin, m_View->angles, m_View->zNear, m_View->zFar, m_View->fov,
                               m_View->m_flAspectRatio, viewFrustum);
    ...
    for (int i = 0; i < glowMgr->m_GlowObjectDefinitions.Count(); i++)
    {
        ...
        // Glow mode
        {
            ...
            if (ce_outlines_pvs_optimizations.GetBool() && currentExtra.m_Mode != GlowMode::Always &&
            ...
            if (player)
            {
                ...
                if (team == TFTeam::Red || team == TFTeam::Blue)
                {
                    ...
                    if (infillsEnable)
                    {
                        ...
                        if (Test_PlaneHitboxesIntersect(player->GetBaseAnimating(), viewFrustum, worldToScreen,
                                                        screenMins, screenMaxs))
                        {
                            ...
                            const auto overhealPercentage = RemapValClamped(playerHealth, playerMaxHealth,
                                                                            int(playerMaxHealth * 1.5 / 5) * 5, 0, 1);
                            ...
                            if (overhealPercentage <= 0)
                            {
                                if (healthPercentage != 1)
                                {
                                    ...
                                    hurtInfill.m_Color.a() *=
                                        ApplyInfillTimeEffects(player->GetState<PlayerHealthState>()
                                                                   .GetLastHurtTime()); // Infill fading/flickering

                                    hurtInfill.m_RectMax.Init(bInfillDebug ? screenMaxs.x : m_View->width,
                            ...
                            else
                            {
                                ...
                                buffedInfill.m_RectMax.Init(bInfillDebug ? screenMaxs.x : m_View->width,
    ...
    if (infillsEnable)
    {
        std::sort(m_ExtraGlowData.begin(), m_ExtraGlowData.end(),
                  [](const ExtraGlowData& lhs, const ExtraGlowData& rhs) -> bool {
    ...
    {
        int conIndex = 0;
        engine->Con_NPrintf(conIndex++, "Glow object count: %zi/%i", m_ExtraGlowData.size(),
                            glowMgr->m_GlowObjectDefinitions.Count());
        for (int i = 0; i < glowMgr->m_GlowObjectDefinitions.Count(); i++)
        {
            auto& current = glowMgr->m_GlowObjectDefinitions[i];
            ...
            else
            {
                ...
                engine->Con_NPrintf(conIndex++, "%sGlow object: %s [i %i, extra %i, entindex %i]", prefix,
                                    ent->GetClientClass()->GetName(), i, extraIndex, ent->entindex());
            }
        }
...
void Graphics::DrawInfills(CMatRenderContextPtr& pRenderContext)
{
    ...
    CRefPtrFix<IMaterial> infillMaterial(materials->FindMaterial(
        ce_infills_additive.GetBool() ? "vgui/white_additive" : "vgui/white", TEXTURE_GROUP_OTHER, true));
...
#define DRAW_INFILL_VGUI 0
    ...
    pRenderContext->Ortho(pixelOffset, pixelOffset, m_View->width + pixelOffset, m_View->height + pixelOffset, -1.0f,
                          1.0f);
...
void CGlowObjectManager::GlowObjectDefinition_t::DrawModel()
{
    ...
    if (ent)
    {
        ...
        AssertMsg(initialColor == GetColorModulation(), "Color mismatch after drawing %s",
                  ent->GetClientClass()->GetName());
        ...
        for (auto moveChild : extra->m_MoveChildren)
        {
            ...
            AssertMsg(initialColor == GetColorModulation(), "Color mismatch after drawing %s",
                      moveChild->GetClientClass()->GetName());
        }
        ...
        while (pAttachment != NULL)
        {
            if (pAttachment->ShouldDraw())
            {
                ...
                AssertMsg(initialColor == GetColorModulation(), "Color mismatch after drawing %s",
                          pAttachment->GetClientClass()->GetName());
            }
...
void Graphics::DrawGlowAlways(int nSplitScreenSlot, CMatRenderContextPtr& pRenderContext) const
{
    ...
    {
        if (current.m_Mode != GlowMode::Always || current.m_Base->IsUnused() ||
            !current.m_Base->ShouldDraw(nSplitScreenSlot))
            continue;
...
static ConVar glow_outline_effect_stencil_mode(
    "glow_outline_effect_stencil_mode", "0", 0,
    "\n\t0: Draws partially occluded glows in a more 3d-esque way, making them look more like they're actually "
...
{
    VPROF_BUDGET(__FUNCTION__, VPROF_BUDGETGROUP_CE);
#if ADDED_OVERRIDE_DEPTH_FUNC // Enable this when the TF2 team has added IMatRenderContext::OverrideDepthFunc or
                              // similar.
    ShaderStencilState_t stencilState;
    stencilState.m_bEnable = true;
    stencilState.m_nReferenceValue = 1;
    stencilState.m_CompareFunc = STENCILCOMPARISONFUNCTION_ALWAYS;
    stencilState.m_PassOp =
        glow_outline_effect_stencil_mode.GetBool() ? STENCILOPERATION_KEEP : STENCILOPERATION_REPLACE;
    stencilState.m_FailOp = STENCILOPERATION_KEEP;
    stencilState.m_ZFailOp = STENCILOPERATION_REPLACE;
    stencilState.SetStencilState(pRenderContext);
    ...
    for (int i = 0; i < glowObjectDefinitions.Count(); i++)
    {
        ...
        if (current.IsUnused() || !current.ShouldDraw(nSplitScreenSlot) || !current.m_bRenderWhenOccluded ||
            current.m_bRenderWhenUnoccluded)
            continue;
    ...
    // Draw depthtest-passing pixels to the stencil buffer
    {
        ...
        for (const auto& current : m_ExtraGlowData)
        {
            if (current.m_Mode != GlowMode::Occluded || current.m_Base->IsUnused() ||
                !current.m_Base->ShouldDraw(nSplitScreenSlot))
                continue;
...
void Graphics::DrawGlowVisible(int nSplitScreenSlot, CMatRenderContextPtr& pRenderContext) const
{
    ...
    for (const auto& current : m_ExtraGlowData)
    {
        if (current.m_Mode != GlowMode::Unoccluded || current.m_Base->IsUnused() ||
            !current.m_Base->ShouldDraw(nSplitScreenSlot))
            continue;
...
void CGlowObjectManager::ApplyEntityGlowEffects(const CViewSetup* pSetup, int nSplitScreenSlot,
                                                CMatRenderContextPtr& pRenderContext, float flBloomScale, int x, int y,
                                                int w, int h)
{
    VPROF_BUDGET(__FUNCTION__, VPROF_BUDGETGROUP_CE);
    ...
    // Draw glow models
    {
        ...
        CRefPtrFix<IMaterial> pGlowColorMaterial(
            materials->FindMaterial("castingessentials/outlines/color_override_material", TEXTURE_GROUP_OTHER, true));
    ...
    {
#if FIXED_COPY_TEXTURE_TO_RENDER_TARGET // Coordinates don't seem to be mapped 1:1 properly, screen becomes slightly
                                        // blurry
        pRenderContext->CopyTextureToRenderTargetEx(0, pRtFullFrameFB1, nullptr);
#else
        pRenderContext->SetStencilEnable(false);

        CRefPtrFix<IMaterial> backbufferReloadMaterial(materials->FindMaterial(
            "castingessentials/outlines/backbuffer_reload_from_fb1", TEXTURE_GROUP_RENDER_TARGET));
        pRenderContext->Bind(backbufferReloadMaterial);

        pRenderContext->OverrideDepthEnable(true, false);
        {
            pRenderContext->DrawScreenSpaceRectangle(
                backbufferReloadMaterial, 0, 0, nViewportWidth, nViewportHeight, 0, 0, nSrcWidth - 1, nSrcHeight - 1,
    ...
    // Bloom glow models from _rt_FullFrameFB0 to backbuffer while stenciling out inside of models
    {
        ...
        {
            CRefPtrFix<IMaterial> finalBlendMaterial(
                materials->FindMaterial("castingessentials/outlines/final_blend", TEXTURE_GROUP_RENDER_TARGET));
            finalBlendMaterial->FindVar("$basetexture", nullptr)->SetTextureValue(pRtFullFrameFB0);

            pRenderContext->DrawScreenSpaceRectangle(finalBlendMaterial, 0, 0, nViewportWidth, nViewportHeight, 0, 0,
                                                     nSrcWidth - 1, nSrcHeight - 1, pRtFullFrameFB1->GetActualWidth(),
                                                     pRtFullFrameFB1->GetActualHeight());
        }
        else
        {
            ...
            //==================================//
            {
                CRefPtrFix<IMaterial> pPoissonExpand(
                    materials->FindMaterial("castingessentials/outlines/poisson_outline", TEXTURE_GROUP_OTHER, true));
                ...
                // Draw quad
                pRenderContext->DrawScreenSpaceRectangle(pPoissonExpand, 0, 0, nSrcWidth, nSrcHeight, 0, 0,
                                                         nSrcWidth - 1, nSrcHeight - 1, srcTexture->GetActualWidth(),
                                                         srcTexture->GetActualHeight());
            ...
            if (const auto BLUR = graphicsModule->ce_outlines_blur.GetFloat(); BLUR >= 1)
            {
                ...
                // Downsampling
                {
                    ...
                    for (; inSize.x / 2 > targetSize.x; inSize /= 2, outSize /= 2)
                    {
                        ...
                        pRenderContext->DrawScreenSpaceRectangle(
                            matDownsample, 0, 0, outSize.x, outSize.y, 0, 0, inSize.x - 1, inSize.y - 1,
                    ...
                    {
                        CRefPtrFix<IMaterial> matCopy(materials->FindMaterial("castingessentials/outlines/direct_copy",
                        ...
                        pRenderContext->DrawScreenSpaceRectangle(
                            matCopy, 0, 0, targetSize.x, targetSize.y, 0, 0, inSize.x - 1, inSize.y - 1,
                            srcTexture->GetActualWidth(), srcTexture->GetActualHeight());
                ...
                //=================//
                {
                    CRefPtrFix<IMaterial> pMatBlurX(
                        materials->FindMaterial("castingessentials/outlines/l4d_ce_blur_x", TEXTURE_GROUP_OTHER, true));
                    ...
                    pRenderContext->DrawScreenSpaceRectangle(
                        pMatBlurX, 0, 0, std::lround(targetSize.x), std::lround(targetSize.y), 0, 0, targetSize.x - 1,
                        targetSize.y - 1, srcTexture->GetActualWidth(), srcTexture->GetActualHeight());
                ...
                //=================//
                {
                    CRefPtrFix<IMaterial> pMatBlurY(
                        materials->FindMaterial("castingessentials/outlines/l4d_ce_blur_y", TEXTURE_GROUP_OTHER));
                    ...
                    pRenderContext->DrawScreenSpaceRectangle(
                        pMatBlurY, 0, 0, std::lround(targetSize.x), std::lround(targetSize.y), 0, 0, targetSize.x - 1,
                        targetSize.y - 1, srcTexture->GetActualWidth(), srcTexture->GetActualHeight());
            ...
            {
                CRefPtrFix<IMaterial> finalBlend(
                    materials->FindMaterial("castingessentials/outlines/final_blend", TEXTURE_GROUP_RENDER_TARGET));
                ...
                pRenderContext->DrawScreenSpaceRectangle(finalBlend, 0, 0, nViewportWidth, nViewportHeight, 0, 0,
                                                         targetSize.x - 1, targetSize.y - 1,
                                                         srcTexture->GetActualWidth(), srcTexture->GetActualHeight());
            }
        }
    }
...
void Graphics::PlayerHealthState::ResetLastHurtTime() { m_LastHurtTime = Interfaces::GetEngineTool()->ClientTime(); }
