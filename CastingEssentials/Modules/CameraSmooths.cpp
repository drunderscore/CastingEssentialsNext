...
CameraSmooths::CameraSmooths()
    : ce_smoothing_enabled("ce_smoothing_enabled", "0", FCVAR_NONE, "Enables smoothing between spectator targets."),
      ce_smoothing_fov("ce_smoothing_fov", "45", FCVAR_NONE, "Only targets within this FOV will be smoothed to.", true,
                       0, true, 180),
      ce_smoothing_max_distance("ce_smoothing_max_distance", "2250", FCVAR_NONE,
                                "max distance at which smoothing will be performed"),
      ce_smoothing_force_distance("ce_smoothing_force_distance", "128", FCVAR_NONE,
                                  "Always smooth if we're closer than this distance."),

      ce_smoothing_linear_speed("ce_smoothing_linear_speed", "875", FCVAR_NONE,
                                "Speed at which to approach the bezier curve start."),
      ce_smoothing_bezier_dist("ce_smoothing_bezier_dist", "1000", FCVAR_NONE,
                               "Units from target to begin the bezier smooth."),
      ce_smoothing_bezier_duration("ce_smoothing_bezier_duration", "0.65", FCVAR_NONE,
                                   "Duration over which to smooth the camera."),

      ce_smoothing_ang_bias("ce_smoothing_ang_bias", "0.85", FCVAR_NONE,
                            "biasAmt for angle smoothing. 1 = linear, 0 = sharp snap at halfway", true, 0, true, 1),
      ...
      ce_smoothing_debug("ce_smoothing_debug", "0", FCVAR_NONE,
                         "Prints debugging info about camera smooths to the console."),
      ce_smoothing_debug_los("ce_smoothing_debug_los", "0", FCVAR_NONE,
                             "Draw points associated with LOS checking for camera smooths."),

      ce_smoothing_check_los("ce_smoothing_check_los", "1", FCVAR_NONE,
                             "Make sure we have LOS to the player we're smoothing to."),
      ce_smoothing_los_buffer("ce_smoothing_los_buffer", "32", FCVAR_NONE,
                              "Additional space to give ourselves so we can sorta see around corners."),
      ce_smoothing_los_min(
          "ce_smoothing_los_min", "0", FCVAR_NONE,
          "Minimum percentage of points that must pass the LOS check before we allow ourselves to smooth to a target.",
          true, 0, true, 1)
{
...
void CameraSmooths::UpdateCollisionTests()
{
    ...
    if (m_CollisionTestFrame != currentFrame)
    {
        ...
        for (Player* player : Player::Iterable())
        {
            ...
            newTest.m_Visibility =
                CameraTools::CollisionTest3D(viewPos, eyePos, ce_smoothing_los_buffer.GetFloat(), entity);
...
void CameraSmooths::DrawCollisionTests()
{
    ...
    for (const CollisionTest& test : m_CollisionTests)
    {
        ...
        NDebugOverlay::Box(Vector(0, 0, 0), test.m_Mins, test.m_Maxs, Lerp(test.m_Visibility, 255, 0),
                           Lerp(test.m_Visibility, 0, 255), 0, 64, 0);
...
static float ComputeSmooth(float time, float startTime, float totalDist, float maxSpeed, float bezierDist,
                           float bezierDuration, float& percent)
{
    // x = time in seconds
...
bool CameraSmooths::SetupEngineViewOverride(Vector& origin, QAngle& angles, float& fov)
{
    ...
    if (hltvcamera->GetMode() == OBS_MODE_IN_EYE || hltvcamera->GetMode() == OBS_MODE_CHASE)
    {
        if (hltvcamera->m_iTraget1 != m_EndTarget || (hltvcamera->GetMode() != m_EndMode && !m_InProgress))
        {
            ...
            const float angle =
                Rad2Deg(std::acosf(currentForward.Dot(deltaPos) / (currentForward.Length() + deltaPos.Length())));
            ...
            if (!forceSmooth)
            {
                ...
                {
                    if (ce_smoothing_debug.GetBool())
                        ConColorMsg(DBGMSG_COLOR,
                                    "[%s] Skipping smooth, distance of %1.0f units > %s (%1.0f units)\n\n",
                                    GetModuleName(), distance, ce_smoothing_max_distance.GetName(),
                                    ce_smoothing_max_distance.GetFloat());
                    m_InProgress = false;
                    return false;
                }
            ...
            else
            {
                if (ce_smoothing_debug.GetBool())
                    ConColorMsg(DBGMSG_COLOR, "[%s] Forcing smooth, distance of %1.0f units < %s (%1.0f units)\n",
                                GetModuleName(), distance, ce_smoothing_force_distance.GetName(),
                                ce_smoothing_force_distance.GetFloat());
            }
    ...
    if (m_InProgress)
    {
        ...
        const float targetDist =
            ComputeSmooth(hosttime, m_SmoothStartTime, m_StartDist, ce_smoothing_linear_speed.GetFloat(),
                          ce_smoothing_bezier_dist.GetFloat(), ce_smoothing_bezier_duration.GetFloat(), percent);
