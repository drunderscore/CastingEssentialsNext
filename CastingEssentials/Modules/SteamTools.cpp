...
SteamTools::SteamTools()
    : ce_steamtools_rp_legacy("ce_steamtools_rp_legacy", "", FCVAR_NONE,
                              "The rich presence status displayed in the \"View Game Info\" dialog.",
                              [](IConVar*, const char*, float) { GetModule()->UpdateRichPresence(); }),

      ce_steamtools_rp_state("ce_steamtools_rp_state", "", FCVAR_NONE,
                             "See wiki. Can be empty, or one of MainMenu, Searching[Generic/MatchGroup], "
                             "Playing[Generic/MatchGroup/Community], or Loading[Generic/MatchGroup/Community].",
                             [](IConVar* var, const char* pOldValue, float) {
                                 GetModule()->SetRichPresenceState(static_cast<ConVar*>(var), pOldValue);
                             }),

      ce_steamtools_rp_matchgroup(
          "ce_steamtools_rp_matchgroup", "", FCVAR_NONE,
          "See wiki. Can be empty, or one of Competitive6v6, Casual, SpecialEvent, MannUp, or BootCamp.",
          [](IConVar* var, const char* pOldValue, float) {
              GetModule()->SetMatchGroup(static_cast<ConVar*>(var), pOldValue);
          }),

      ce_steamtools_rp_currentmap("ce_steamtools_rp_currentmap", "", FCVAR_NONE,
                                  "See wiki. Can be used to insert custom text into a rich presence template.",
                                  [](IConVar* var, const char*, float) { GetModule()->UpdateRichPresence(); }),

      ce_steamtools_rp_debug(
          "ce_steamtools_rp_debug", []() { GetModule()->PrintRichPresence(); },
          "Prints the current rich presence state to console.")
{
