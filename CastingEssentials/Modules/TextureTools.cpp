...
TextureTools::TextureTools()
    : ce_texturetools_full_res_rts(
          "ce_texturetools_full_res_rts", "0", FCVAR_NONE,
          "Create the refraction and water reflection textures at framebuffer resolution (if possible). Must be set "
          "before the first map load. Changing after that requires a game restart.",
          [](IConVar*, const char*, float) { GetModule()->ToggleFullResRTs(); }),

      m_CreateRenderTargetsHook(std::bind(&TextureTools::InitClientRenderTargetsOverride, this, std::placeholders::_1,
...
bool TextureTools::CheckDependencies() { return true; }

static ITexture* CreateWaterReflectionTexture(IMaterialSystem* pMaterialSystem, int iSize)
{
    return pMaterialSystem->CreateNamedRenderTargetTextureEx2(
        "_rt_WaterReflection", iSize, iSize, RT_SIZE_FULL_FRAME_BUFFER, pMaterialSystem->GetBackBufferFormat(),
...
{
    return pMaterialSystem->CreateNamedRenderTargetTextureEx2(
        "_rt_WaterRefraction", iSize, iSize, RT_SIZE_FULL_FRAME_BUFFER,
...
void TextureTools::ToggleFullResRTs() { m_CreateRenderTargetsHook.SetEnabled(ce_texturetools_full_res_rts.GetBool()); }

void TextureTools::InitClientRenderTargetsOverride(CBaseClientRenderTargets* pThis, IMaterialSystem* matsys,
                                                   IMaterialSystemHardwareConfig* config, int waterRes, int cameraRes)
{
    ...
    // Full frame depth texture
    {
        ...
        m_FullFrameDepth.Init(materials->CreateNamedRenderTargetTextureEx2(
            "_rt_FullFrameDepth", 1, 1, RT_SIZE_FULL_FRAME_BUFFER, IMAGE_FORMAT_IA88, MATERIAL_RT_DEPTH_NONE,
