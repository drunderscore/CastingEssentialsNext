...
private:
    ...
    {
    public:
        ChargeBarInfo(const char* name, int defaultPriority, const char* helpText, const char* displayText, int itemID,
                      MeterType meter, float effectBarChargeTime = -1)
            : m_Name(name), m_DefaultPriority(defaultPriority), m_HelpText(helpText), m_DisplayText(displayText),
              m_ItemID(itemID), m_Meter(meter), m_EffectBarChargeTime(effectBarChargeTime)
        {
    ...
    private:
        ...
        static std::unique_ptr<char[]> FormatAndAlloc(const char* fmt, const Args&... args);
