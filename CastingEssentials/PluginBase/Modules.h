...
private:
    ...
    virtual const char* GetModuleName() = 0; // ModuleManager gets to call this so it can retrieve module name after
                                             // type erasure
    virtual std::unique_ptr<IBaseModule> ReplaceSingleton(
        std::unique_ptr<IBaseModule> replacement) = 0; // Replaces global singleton and returns the old instance
...
public:
    ...
    {
        if (!s_Module)
            throw module_not_loaded(T::GetModuleName());
...
template<class T>
std::unique_ptr<IBaseModule> Module<T>::s_Module = nullptr; // Static module pointer variable definition
...
struct ModuleDesc
{
    ...
    ModuleDesc(const std::type_info& ti, const char* name, std::function<std::unique_ptr<IBaseModule>()> factory)
        : ti(ti), name(name), factory(factory)
    {
...
#define MODULE_REGISTER(T)                                                                                             \
    ModuleDesc module_desc_##T(typeid(T), T::GetModuleName(), []() -> std::unique_ptr<IBaseModule> {                   \
        if (!T::CheckDependencies())                                                                                   \
            throw module_load_failed(T::GetModuleName());                                                              \
...
public:
    ...
    template<typename ModuleType>
    constexpr const char* GetModuleName() const;
    template<typename ModuleType>
    void Depend();
    void LoadAll();
...
template<typename ModuleType>
inline ModuleType* ModuleManager::GetModule() const
{
    return ModuleType::GetModule();
}

template<typename ModuleType>
inline constexpr const char* ModuleManager::GetModuleName() const
{
    return ModuleType::GetModuleName();
}

template<typename ModuleType>
inline void ModuleManager::Depend()
{
