...
void Player::Load()
{
    ...
    {
        s_UserInfoChangedCallbackHook = GetHooks()->AddHook<HookFunc::Global_UserInfoChangedCallback>(
            std::bind(&Player::UserInfoChangedCallbackOverride, std::placeholders::_1, std::placeholders::_2,
...
{
    if (s_UserInfoChangedCallbackHook &&
        GetHooks()->RemoveHook<HookFunc::Global_UserInfoChangedCallback>(s_UserInfoChangedCallbackHook, __FUNCSIG__))
...
bool Player::CheckDependencies()
{
    ...
    if (!Interfaces::GetEngineClient())
        PluginWarning(
            "Interface IVEngineClient for player helper class not available (required for retrieving certain info)!\n");

    if (!Interfaces::GetHLTVCamera())
    {
        PluginWarning("Interface C_HLTVCamera for player helper class not available (required for retrieving spectated "
                      "player)!\n");
        ready = false;
    }
    ...
    if (!Interfaces::AreSteamLibrariesAvailable())
        PluginWarning(
            "Steam libraries for player helper class not available (required for accuracy in retrieving Steam IDs)!\n");
    ...
    // try
    {
        ...
        for (size_t i = 0; i < s_WeaponOffsets.size(); i++)
            s_WeaponOffsets[i] = Entities::GetEntityProp<CHandle<C_BaseCombatWeapon>>(
                playerClass, Entities::PropIndex(buffer, "m_hMyWeapons", i));
        ...
        if (Interfaces::GetEngineClient()->GetPlayerInfo(GetEntity()->entindex(), &playerInfo))
        {
            if (playerInfo.friendsID)
            {
                ...
                if (universe == k_EUniverseInvalid)
                {
                    ...
                    {
                        // let's just assume that it's public - what are the chances that there's a Valve employee
                        // testing this on another universe without Steam?
...
C_BasePlayer* Player::GetBasePlayer() const { return dynamic_cast<C_BasePlayer*>(GetEntity()); }
...
bool Player::CheckCondition(TFCond condition) const
{
    if (IsValid())
    {
        ...
        if (condition < 32)
            return (s_PlayerCondBitOffsets[0].GetValue(ent) | s_PlayerCondBitOffsets[1].GetValue(ent)) &
                   (1 << condition);
...
int Player::GetMaxOverheal() const { return (int(GetMaxHealth() * 1.5f) / 5) * 5; }
...
void Player::UserInfoChangedCallbackOverride(void*, INetworkStringTable* stringTable, int stringNumber,
...
Player::Iterator Player::end() { return Player::Iterator(Interfaces::GetEngineTool()->GetMaxClients() + 1); }
...
C_BaseCombatWeapon* Player::GetMedigun(TFMedigun* medigunType) const
{
    ...
    for (int i = 0; i < MAX_WEAPONS; i++)
    {
        ...
        if (medigunType)
        {
            ...
            {
                case 29:
                    *medigunType = TFMedigun::MediGun;
                    break;
                case 35:
                    *medigunType = TFMedigun::Kritzkrieg;
                    break;
                case 411:
                    *medigunType = TFMedigun::QuickFix;
                    break;
                case 998:
                    *medigunType = TFMedigun::Vaccinator;
                    break;
                default:
