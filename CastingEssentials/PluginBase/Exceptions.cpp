...
invalid_class_prop::invalid_class_prop(const char* name) noexcept { className = name; }
...
module_not_loaded::module_not_loaded(const char* name) noexcept { moduleName = name; }
...
bad_pointer::bad_pointer(const char* type) noexcept { pointerType = type; }
