	...
	const std::function<bool(std::byte* found)>& testFunc, int offset)
{
	...
	std::byte* found;
	while (found = (std::byte*)FindPattern(reinterpret_cast<uintptr_t>(clientModInfo.lpBaseOfDll) + searchOffset, clientModInfo.SizeOfImage - searchOffset, (PBYTE)signature, mask))
	{
		if (testFunc(found + offset))
			return found + offset;

		searchOffset += reinterpret_cast<uintptr_t>(found) - reinterpret_cast<uintptr_t>(clientModInfo.lpBaseOfDll) - searchOffset + 1;
	}
...
std::byte* SignatureScan(const char* moduleName, const char* signature, const char* mask, int offset)
{
	...
	auto found = (std::byte*)FindPattern(reinterpret_cast<uintptr_t>(clientModInfo.lpBaseOfDll), clientModInfo.SizeOfImage, (PBYTE)signature, mask);
	if (found)
		found += offset;
...
void HookManager::FindFunc_C_BasePlayer_GetLocalPlayer()
{
	...
	s_RawFunctions[(int)HookFunc::C_BasePlayer_GetLocalPlayer] = reinterpret_cast<void*>(localPlayerCall + localPlayerOffset + 5);
}
...
void HookManager::FindFunc_CNewParticleEffect_SetDormant()
{
	...
	s_RawFunctions[(int)HookFunc::CNewParticleEffect_SetDormant] = reinterpret_cast<void*>(setDormantCall + setDormantOffset + 5);
}
...
void HookManager::InitRawFunctionsList()
{
	...
	FindFunc<HookFunc::Global_CreateTFGlowObject>("", "xxxxxxxxxxxxxxxxxxxxxxxxx????xxxxxxx?????xxxxxxxxx????x??????xxxxxxxxx??????xxxxxxxxxxxxx");
	...
	FindFunc<HookFunc::Global_GetLocalPlayerIndex>("\x48\x83\xEC\x28\xE8\x00\x00\x00\x00\x48\x85\xC0\x74\x00\x48\x8D\x48\x10\x48\x8B\x40\x10\x48\x83\xC4\x28\x48\xFF\x60\x48\x48\x83\xC4\x28\xC3", "xxxxx????xxxx?xxxxxxxxxxxxxxxxxxxxx");
	...
	FindFunc<HookFunc::Global_UserInfoChangedCallback>("\x48\x89\x6C\x24\x18\x56\x48\x83\xEC\x20\x48\x8B\x74\x24\x50\x41\x8B\xE8\x48\x85\xF6\x0F\x00\x00\x00\x00\x00", "xxxxxxxxxxxxxxxxxxxxxx?????", 0, "engine");
	FindFunc<HookFunc::Global_UTILComputeEntityFade>("\x4C\x8B\xDC\x49\x89\x73\x18\x55\x49\x8D\x6B\xA1\x48\x81\xEC\xB0\x00\x00\x00", "xxxxxxxxxxxxxxxxxxx");
	FindFunc<HookFunc::Global_UTIL_TraceLine>("\x4C\x8B\xDC\x49\x89\x5B\x08\x57\x48\x81\xEC\xB0\x00\x00\x00\xF3\x0F\x10\x42\x08\x49\x8B\xC1\xF3\x0F\x10\x19\x41\x8B\xF8\xF3\x0F\x10\x0A", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
	...
	FindFunc<HookFunc::C_BaseAnimating_GetBoneCache>("\x48\x89\x5C\x24\x08\x48\x89\x74\x24\x10\x57\x48\x83\xEC\x50\x48\x8B\xF9\x48\x8B\xF2\x48\x8B\x89\xA0\x0B\x00\x00", "xxxxxxxxxxxxxxxxxxxxxxxxxxxx");
	FindFunc<HookFunc::C_BaseAnimating_GetBonePosition>("\x48\x89\x5C\x24\x08\x48\x89\x6C\x24\x10\x48\x89\x74\x24\x18\x57\x48\x83\xEC\x50\x8B\xDA\x49\x8B\xF1\x33\xD2\x49\x8B\xE8\x48\x8B\xF9", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
	FindFunc<HookFunc::C_BaseAnimating_GetSequenceActivityName>("\x55\x8B\xEC\x83\x7D\x08\xFF\x56\x8B\xF1\x75\x0A", "xxxxxxxxxxxx");
	FindFunc<HookFunc::C_BaseAnimating_LockStudioHdr>("\x48\x8B\xC4\x56\x48\x83\xEC\x70\x48\x89\x58\x08\x48\x8D\x91\x00\x0C\x00\x00\x48\x8B\xD9", "xxxxxxxxxxxxxxxxxxxxxx");
	FindFunc<HookFunc::C_BaseAnimating_LookupBone>("\x40\x53\x48\x83\xEC\x20\x48\x8B\xDA\xE8\x00\x00\x00\x00\x48\x8B\xC8\x48\x8B\xD3\x48\x83\xC4\x20\x5B\xE9\x00\x00\x00\x00", "xxxxxxxxxx????xxxxxxxxxxxx????");

	FindFunc<HookFunc::C_BaseEntity_Init>("\x48\x89\x5C\x24\x08\x48\x89\x74\x24\x18\x57\x48\x83\xEC\x30\x48\x8B\x01\x41\x8B\xF0\x8B\xFA\x89\x91\x88\x00\x00\x00", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
	...
	FindFunc<HookFunc::C_BasePlayer_ShouldDrawThisPlayer>("\x48\x83\xEC\x28\xE8\x00\x00\x00\x00\x84\xC0\x74\x00\x48\x00\x00\x00\x00\x00\x00\x48\x85\xC9\x74\x00\x48\x8B\x01\xFF\x50\x30", "xxxxx????xxx?x??????xxxx?xxxxxx");

	FindFunc<HookFunc::C_HLTVCamera_SetMode>("\x48\x89\x74\x24\x10\x57\x48\x83\xEC\x20\x8B\x71\x10\x48\x8B\xF9\x3B\xF2\x74\x00\x89\x51\x10\x45\x33\xC0\x48\x00\x00\x00\x00\x00\x00\x48\x00\x00\x00\x00\x00\x00\x48\x00\x00\x00\x00\x48\x8B\x01\xFF\x50\x00\x48\x8B\xD8\x48\x85\xC0\x74\x00", "xxxxxxxxxxxxxxxxxxx?xxxxxxx??????x??????x????xxxxx?xxxxxxx?");
	FindFunc<HookFunc::C_HLTVCamera_SetPrimaryTarget>("\x48\x89\x5C\x24\x20\x55\x48\x83\xEC\x40\x8B\x69\x30\x48\x8B\xD9\x3B\xEA\x0F\x84\x00\x00\x00\x00\x89\x51\x30\x8B\x49\x14\x48\x89\x74\x24\x50\x48\x89\x7C\x24\x60\x85\xC9\x7E\x00\xE8\x00\x00\x00\x00\x48\x85\xC0\x74\x00", "xxxxxxxxxxxxxxxxxxxx????xxxxxxxxxxxxxxxxxxx?x????xxxx?");

	FindFunc<HookFunc::C_TFPlayer_DrawModel>("\x55\x8B\xEC\x51\x57\x8B\xF9\x80\x7F\x54\x17", "xxxxxxxxxxx");
	FindFunc<HookFunc::C_TFPlayer_GetEntityForLoadoutSlot>("\x48\x89\x5C\x24\x08\x48\x89\x6C\x24\x10\x48\x89\x74\x24\x18\x57\x41\x54\x41\x55\x41\x56\x41\x57\x48\x83\xEC\x30\x8D\x42\xF9\x33\xF6\x44\x8B\xE2\x4C\x8B\xE9\x41\xBE\xFF\x1F\x00\x00", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");

	FindFunc<HookFunc::C_TFWeaponBase_PostDataUpdate>("\x48\x89\x5C\x24\x08\x48\x89\x6C\x24\x10\x48\x89\x74\x24\x18\x57\x48\x83\xEC\x20\x48\x8B\xF1\x8B\xEA\x48\x83\xC1\xF0\xE8\x00\x00\x00\x00\x48\x8B\xF8\x48\x85\xC0", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx????xxxxxx");
	...
	FindFunc<HookFunc::CDamageAccountPanel_FireGameEvent>("\x48\x89\x5C\x24\x10\x48\x89\x6C\x24\x18\x56\x57\x41\x54\x41\x56\x41\x57\x48\x81\xEC\xC0\x00\x00\x00\x48\x8B\x02", "xxxxxxxxxxxxxxxxxxxxxxxxxxxx");
	FindFunc<HookFunc::CDamageAccountPanel_ShouldDraw>("", "xxxxxxxxxx????xxxx?xxxxxxxxxxxxxxx?xxxxxxxxxxx??????xxxxx?xxxxxx");
	...
	FindFunc<HookFunc::CGlowObjectManager_ApplyEntityGlowEffects>("\x48\x8B\xC4\x48\x89\x58\x18\x48\x89\x50\x10\x55\x56\x57\x41\x54\x41\x55\x41\x56\x41\x57\x48\x81\xEC\xB0\x00\x00\x00\x0F\x29\x70\xB8", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");

	FindFunc<HookFunc::CHudBaseDeathNotice_GetIcon>("\x40\x53\x48\x81\xEC\x20\x01\x00\x00\x48\x8B\xDA\x45\x85\xC0\x74\x00\x41\xB8\x02\x00\x00\x00", "xxxxxxxxxxxxxxxx?xxxxxx");

	FindFunc<HookFunc::CStudioHdr_GetNumSeq>("\x48\x8B\x41\x08\x48\x85\xC0\x75\x00\x48\x8B\x01\x8B\x80\xBC\x00\x00\x00\xC3", "xxxxxxxx?xxxxxxxxxx");
	FindFunc<HookFunc::CStudioHdr_pSeqdesc>("\x48\x89\x74\x24\x10\x57\x48\x83\xEC\x20\x33\xF6\x48\x8D\x79\x08\x44\x8B\xCA\x85\xD2\x78\x00\x4C\x8B\x07", "xxxxxxxxxxxxxxxxxxxxxx?xxx");

	FindFunc<HookFunc::CViewRender_PerformScreenSpaceEffects>("\x4C\x8B\xDC\x49\x89\x5B\x08\x49\x89\x6B\x10\x49\x89\x73\x18\x49\x89\x7B\x20\x41\x56\x48\x83\xEC\x60\x48\x00\x00\x00\x00\x00\x00\x33\xFF\x49\x89\x7B\xE8\x41\x8B\xF1\x41\x8B\xE8\x44\x8B\xF2", "xxxxxxxxxxxxxxxxxxxxxxxxxx??????xxxxxxxxxxxxxxx");
	...
	FindFunc<HookFunc::IGameSystem_Add>("\x48\x89\x5C\x24\x08\x48\x89\x74\x24\x10\x57\x48\x83\xEC\x00\x8B\x00\x00\x00\x00\x00\x48\x8B\xF9\x8B\x00\x00\x00\x00\x00\x8B\xDA\xBE\x04\x00\x00\x00\x44\x8D\x42\x01\x44\x3B\xC0\x0F\x8E\x00\x00\x00\x00", "xxxxxxxxxxxxxx?x?????xxxx?????xxxxxxxxxxxxxxxx????");

	FindFunc<HookFunc::vgui_AnimationController_StartAnimationSequence>("\x44\x88\x4C\x24\x20\x53\x41\x54\x41\x55\x41\x56\x48\x83\xEC\x58\x80\xB9\xEB\x01\x00\x00\x00\x49\x8B\xD8\x4C\x8B\xEA\x4C\x8B\xF1\x74\x00\xE8\x00\x00\x00\x00\x4C\x8B\xC3", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx?x????xxx");
	FindFunc<HookFunc::vgui_Panel_FindChildByName>("\x48\x8B\xC4\x53\x48\x81\xEC\xA0\x00\x00\x00\x48\x89\x70\xF0\x33\xF6\x48\x89\x78\xE8\x4C\x89\x60\xE0\x4C\x8B\xE1\x4C\x89\x68\xD8\x45\x0F\xB6\xE8", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
	FindFunc<HookFunc::vgui_ProgressBar_ApplySettings>("\x48\x89\x5C\x24\x08\x48\x89\x74\x24\x10\x57\x48\x83\xEC\x20\x48\x8B\xF2\x48\x8B\xF9\x48\x8B\xCE\x48\x00\x00\x00\x00\x00\x00\x0F\x57\xD2\xE8\x00\x00\x00\x00\x4C\x00\x00\x00\x00\x00\x00\xF3\x0F\x11\x87\xF4\x01\x00\x00", "xxxxxxxxxxxxxxxxxxxxxxxxx??????xxxx????x??????xxxxxxxx");

	FindFunc<HookFunc::CStorePanel_RequestPricesheet>("", "xxxxxxx????xxxxxxxxxxxxxxx????xxxxxxx?xxxxxx????xxxx????xxxxxxxxxxxxxx????x??????xxxxxxxxxxxxxx");

	FindFunc<HookFunc::CParticleProperty_OwnerSetDormantTo>("\x48\x8B\x4E\x10\x40\x0F\xB6\xD5\x48\x8B\x4C\x0B\x20\xE8\x00\x00\x00\x00\x48\x8D\x5B\x28\x48\x83\xEF\x01", "xxxxxxxxxxxxxx????xxxxxxxx");

	FindFunc<HookFunc::C_BaseAnimating_GetSequenceActivity>("\x48\x89\x5C\x24\x08\x57\x48\x83\xEC\x20\x8B\xDA\x48\x8B\xF9\x83\xFA\xFF\x74", "xxxxxxxxxxxxxxxxxxx");
