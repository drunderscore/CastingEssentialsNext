...
protected:
	...
	{
		typedef void(* Raw)(C_BaseAnimating* pThis, ClientModelRenderInfo_t* pInfo, DrawModelState_t* pState, matrix3x4_t* pBoneToWorldArray);
	...
	{
		typedef bool(* Raw)(vgui::AnimationController* pThis, vgui::Panel* pWithinParent, const char* sequenceName, bool unknown);
	};
	...
	{
		typedef void( *Raw)(CGlowObjectManager*, const CViewSetup*, int, CMatRenderContextPtr&, float, int, int, int, int);
	...
	{
		typedef void(* Raw)(CBaseClientRenderTargets* pThis, IMaterialSystem* pMaterialSystem, IMaterialSystemHardwareConfig* config, int iWaterTextureSize, int iCameraTextureSize);
	...
	{
		typedef void(* Raw)(void*, INetworkStringTable* stringTable, int stringNumber, const char* newString, const void* newData);
	...
	{
		typedef void(* Raw)(const Vector& vecAbsStart, const Vector& vecAbsEnd, unsigned int mask, const IHandleEntity* ignore, int collisionGroup, trace_t* ptr);
	...
	template<> struct HookFuncType<HookFunc::CParticleProperty_OwnerSetDormantTo>
	{
		typedef void(*Raw)(CParticleProperty* pThis, bool bDormant);
	};
	template<> struct HookFuncType<HookFunc::C_BaseAnimating_GetSequenceActivity>
	{
