...
bool ColorFromConVar(const ConVar& var, Color& out) { return ColorFromString(var.GetString(), out); }

Color ColorFromConVar(const ConVar& var, bool* success) { return ColorFromString(var.GetString(), success); }
...
Vector ColorToVector(const Color& color) { return Vector(color.r() / 255.0f, color.g() / 255.0f, color.b() / 255.0f); }
...
bool ParseVector(Vector& v, const char* str) { return ParseFloat3(str, v.x, v.y, v.z); }
